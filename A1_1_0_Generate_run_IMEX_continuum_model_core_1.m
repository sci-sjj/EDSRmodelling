% This code creates the IMEX continuum model for core 1 from the PNM files. It
...
%         figure
%         plot(pc{i,j,k}(1:10,1), pc{i,j,k}(1:10,2), 'o-')
         %plot(pc{i,j,k}(end-10:end,1), pc{i,j,k}(end-10:end,2), 'o-')
...
kabs(:,:,2:Nz+1) = kabs(:,:,:);
kabs(:,:,[1,Nz+2]) = kabs(:,:,[2,Nz])*0 + kabs_av*10;

porosity(:,:,2:Nz+1) = porosity(:,:,:);
porosity(:,:,[1,Nz+2]) = porosity(:,:,[2,Nz])*0 + porosity_av;
...
Nt = 3; % Number of fractional flow steps in the experiment
...
sat_av(1:2,1) = 1 - nanmean(nanmean(nanmean(satMat_sim(:,:,2:Nz-1,2:3))));
sat_av_exp(1:2,1) = 1 - nanmean(nanmean(nanmean(Core1_saturations_SJJ(:,:,:,1:2))));
...
q_g = [0,6.00181860020217e-11,1.20036372004043e-09]';
q_w = [1.20036372004043e-09,1.14034553403841e-09,0]';
...
plot((sat_av(1:Nt-1)), kr_g(1:Nt-1), 'o','color', colours (2,:),'MarkerEdge',colours (2,:),'MarkerSize',msize)
plot((sat_av(1:Nt-1)), kr_w(1:Nt-1),'d','color', colours (2,:),'MarkerEdge',colours (2,:),'MarkerSize',msize)
...
L = legend('Oil $k_{ro}$ exp 1', 'Water $k_{rw}$ exp 1', 'Oil $k_{ro}$ exp 2', 'Water $k_{rw}$ exp 2', 'Oil $k_{ro}$ exp 3','Water $k_{rw}$ exp 3', 'Oil $k_{ro}$ sim 2','Water $k_{rw}$ sim 2','BC Oil $k_{ro}$','BC Water $k_{rw}$','Location', 'EastOutside', 'fontsize', 12)
L.Interpreter = 'latex';
...
bb = satMat_sim(:,:,2:37,2);

sol(1,9)= nansum((abs(aa(:) - bb(:))))/864;
sol(1,10)= sqrt(nansum(((aa(:) - bb(:)).^2))/864);
...
bb = satMat_sim(:,:,2:37,3);
plot(aa(:), bb(:),'o', 'color', colours (5,:),'MarkerEdge',colours (5,:),'MarkerFace',colours (5,:),'MarkerSize',msize)


sol(2,9)= nansum((abs(aa(:) - bb(:))))/864;
sol(2,10)= sqrt(nansum(((aa(:) - bb(:)).^2))/864);

plot(linspace(0, 1, 100), linspace(0, 1, 100), 'k-', 'linewidth', 2)

L = legend('$f_o = 0.05$', '$f_o = 1$' ,'Location', 'SouthEast', 'Interpreter', 'latex', 'fontsize', 14);
...
bb =  squeeze(nanmean(nanmean(satMat_sim(:,:,2:37,2))))';
plot(x_loc_exp3/73.2,aa,'o--', 'color', colours (3,:),'MarkerEdge',colours (3,:),'MarkerFace',colours (3,:),'MarkerSize',msize)
...
bb =  squeeze(nanmean(nanmean(satMat_sim(:,:,2:37,3))))';
plot(x_loc_exp3/73.2, aa,'o--', 'color', colours (5,:),'MarkerEdge',colours (5,:),'MarkerFace',colours (5,:),'MarkerSize',msize)
plot(x_loc_exp3/73.2,bb, '-','linewidth', 2, 'color', colours (5,:),'MarkerEdge',colours (5,:),'MarkerFace',colours (3,:),'MarkerSize',msize)


L = legend('Exp $f_o = 0.05$','Sim $f_o = 0.05$','Exp $f_o = 1$','Sim $f_o = 1$','Location', 'NorthEast', 'Interpreter', 'latex', 'fontsize', 12);
...
sol(:,4) = kro_exp3(1:2,2);
sol(:,5) = kr_g';

sol(:,6) = krw_exp3(1:2,2);
sol(:,7) = kr_w';

sol(:,8) = (sol(:,3) - sol(:,1))./sol(:,1)*100;

sol(:,11)= (sol(:,5) - sol(:,4))./sol(:,4)*100;
sol(:,12) = (sol(:,7) - sol(:,6))./sol(:,6)*100;
